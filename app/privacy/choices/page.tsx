    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 z-10">
        <div className="mx-auto max-w-3xl px-6 py-12">
          ...
          {lang==='ko' ? (
            <>
              <h1 className="text-3xl font-semibold text-neon-green drop-shadow-[0_0_10px_rgba(0,255,136,0.5)] font-display">개인정보 선택 설정</h1>
              <p className="mt-3 text-sm text-muted-foreground">아래에서 개인정보 처리와 관련된 선택 사항을 확인하세요. 일부 항목은 앱 내 설정 또는 문의를 통해 처리됩니다.</p>
            </>
          ) : (
            <>
              <h1 className="text-3xl font-semibold text-neon-green drop-shadow-[0_0_10px_rgba(0,255,136,0.5)] font-display">Privacy Choices</h1>
          ...
          <section className="mt-8 space-y-3">
            <h2 className="text-xl font-medium text-neon-cyan">{lang==='ko' ? '1. 푸시 알림 수신 설정' : '1. Push Notification Preferences'}</h2>
          ...
          <section className="mt-8 space-y-3">
            <h2 className="text-xl font-medium text-neon-magenta">{lang==='ko' ? '2. 계정 정보 수정/삭제' : '2. Edit/Delete Account Info'}</h2>
          ...
          <section className="mt-8 space-y-3">
            <h2 className="text-xl font-medium text-neon-pink">{lang==='ko' ? '3. 데이터 접근·정정·삭제 요청' : '3. Data Access/Correction/Deletion'}</h2>
            ...
            <div className="rounded-lg border border-white/10 bg-white/5 p-4 text-sm text-muted-foreground">
              {lang==='ko' ? '이메일' : 'Email'}: <a className="underline text-neon-cyan hover:text-neon-cyan/80" href={`mailto:${supportEmail}`}>{supportEmail}</a>
            </div>
          </section>

          <section className="mt-8 space-y-3">
            <h2 className="text-xl font-medium text-neon-yellow">{lang==='ko' ? '4. 이메일 수신 동의/철회' : '4. Email Consent/Opt-out'}</h2>
